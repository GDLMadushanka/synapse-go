...
func DeploySequences(folderPath string, confContext *artifacts.ArtifactInfo) {
	...
	if len(files) == 0 {
		consolelogger.DebugLog("No sequences found. Add your sequences to the artifacts/Sequences folder")
	}
	...
	for _, file := range files {
		...
		if err := xml.Unmarshal([]byte(data), &sequence); err != nil {
