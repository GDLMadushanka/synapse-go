...
func DeployInbound(folderPath string, confContext *artifacts.ArtifactInfo, context context.Context) {
	...
	if len(files) == 0 {
		consolelogger.DebugLog("No Inbounds found. Add your Inbounds to the artifacts/InboundEndpoints folder")
	}
	...
	for _, file := range files {
		...
		fileInboundEndpoint, err := services.GetInstance(inbound)
		if err == nil {
			waitgroup.Add(1)
			go fileInboundEndpoint.PollFile(context)
		}
	}
}
